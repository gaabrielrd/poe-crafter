  ...
  it('monta a rota raiz com o layout e a pagina inicial', async () => {
    ...
    expect(
      screen.getByRole('heading', {
        name: 'Uma bancada confiável para planejar cada craft.',
        level: 1,
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('heading', { name: 'Comandos pnpm disponíveis', level: 2 }),
    ).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'Página inicial' })).toHaveTextContent(
      'PoE Crafting Planner',
    );
    expect(screen.queryByRole('textbox', { name: 'Título da nota' })).not.toBeInTheDocument();
  });
